...
if start_idx != -1:
    ...
    if items:
        # Keep just 5 items for a perfect 3x2 grid layout with one large item (2 cols) + 4 small (1 col each)
        five_items = '\n      ' + '\n      '.join(items[:5]) + '\n    '
        text = text[:content_start] + five_items + text[gallery_end:]
        
        with open('index.html', 'w', encoding='utf-8') as f:
            f.write(text)
        print('Items truncated to 5 successfully')
